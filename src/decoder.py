...
class Decoder(BaseModel):
    ...
    def get_valid_tokens(self, state: DecoderState, function_def: FunctionDefinition,
                     ...
                     param_name_tokens: dict[str, list[int]] = {}) -> list[int]:
        ...
        if state == DecoderState.START:
            ...
            if param_type in ["number", "integer"]:
                ...
                separator_tokens = self.model.encode(', ').tolist()[0] + self.model.encode('}').tolist()[0]
                ...
                separator_tokens = self.model.encode(', ').tolist()[0] + self.model.encode('}').tolist()[0]
                return list(range(len(self.vocabulary.id_to_token))) + separator_tokens
    ...
    def generate(self, prompt: str, function_def: FunctionDefinition) -> dict:
        ...
        param_name_tokens    = {
            p: self.model.encode(f'"{p}"').tolist()[0]
            for p in function_def.parameters.keys()
        }
        print(f"param_name_tokens: {param_name_tokens}")
        print(f"decoded: { {p: [self.vocabulary.id_to_token.get(t) for t in tokens] for p, tokens in param_name_tokens.items()} }")
        ...
        while state != DecoderState.DONE:
            ...
            print(f"state={state.value} | token='{decoded_token}' | state_tokens={state_tokens} | current_prm='{current_prm}'")
            ...
            if state == DecoderState.PARAM_VALUE and new_state == DecoderState.PARAM_NAME:
                filled_params += 1
                param_index   += 1
        ...
        print(f"\nGenerated string: {repr(json_str)}")
        return json.loads(json_str)
    
    
if __name__ == "__main__":
    from pathlib import Path
    from models import ParameterSpec, ReturnSpec
    ...
    result = decoder.generate(prompt_str, function)
    print(f"\nResult: {result}")